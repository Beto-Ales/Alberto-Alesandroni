...
const App = () => {
  ...
  const addPersons = (event) => {
            ...
            if(update){
              ...
              personService
                ...
                .catch(error => {
                  ...
                  // setErrorMessage(error.response.data.error)   // Information of ${person.name} has already been removed from the server
    ...
    personService
      ...
      .catch(error => {
        setErrorMessage(`${error.response.data.error} `)    // --- name has to be at least three characters long
        setTimeout(() => {
          setErrorMessage(null)
        }, 5000)
      })    
